...
namespace HumanResource.API.Services.Implementations
{
    public class EmployeeService : IEmployeeService
    {
        ...
        public async Task<IEnumerable<EmployeeDto>> GetByJobAsync(string jobId)
        {
            var employees = await _employeeRepository.GetByJobAsync(jobId);
            return _mapper.Map<IEnumerable<EmployeeDto>>(employees);
        }
        public async Task<IEnumerable<EmployeeDto>> GetByRoleAsync(int roleId)
        {
            var employees = await _employeeRepository.GetByRoleAsync(roleId);
            return _mapper.Map<IEnumerable<EmployeeDto>>(employees);
        }
        ...
        public async Task<IEnumerable<EmployeeDto>> GetPaginatedAsync(int pageNumber, int pageSize)
        {
            var employees = await _employeeRepository.GetPaginatedAsync(pageNumber, pageSize);
            return _mapper.Map<IEnumerable<EmployeeDto>>(employees);
        }
        public async Task<EmployeeDto> GetHighestSalaryEmployeeAsync()
        {
            var employee = await _employeeRepository.GetHighestSalaryEmployeeAsync();
