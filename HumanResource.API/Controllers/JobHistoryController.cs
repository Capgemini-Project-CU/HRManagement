...
namespace HumanResource.API.Controllers
{
    ...
    public class JobHistoryController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<IActionResult> AddJobHistory(JobHistoryDto jobHistoryDto)
        {
            var createdJobHistory = await _jobHistoryService.AddAsync(jobHistoryDto);
        ...
        public async Task<IActionResult> GetJobHistoryByDepartment(int departmentId)
        {
            var jobHistories = await _jobHistoryService.GetByDepartmentAsync(departmentId);
